...
async function signBiliBili() {
	// ---- 随机延时签到：0~10分钟内随机延时，模拟真实用户行为，避免固定时间触发风控 ----
	const randomDelay = Math.floor(Math.random() * 10 * 60 * 1000) // 0 ~ 10分钟（毫秒）
	const delayMin = Math.floor(randomDelay / 60000)
	const delaySec = Math.floor((randomDelay % 60000) / 1000)
	$.log(`#### 随机延时签到`)
	$.log(`- 本次将延时 ${delayMin} 分 ${delaySec} 秒后开始执行`)
